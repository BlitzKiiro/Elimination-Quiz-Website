//imports
import { slideUpAnimaion, rightAnswerAnimation, wrongAnswerAnimation, slideDownAnimation, } from "./animations.js";
import Quiz from "./quiz.js";
//query selectors
let start = document.querySelector(".start-icon");
let reset = document.querySelector(".reset-icon");
let [questionTitle, answersDiv] = Array.from(document.querySelector(".questions-area").children);
...
const renderNextQuestion = () => {
    ...
    answers.forEach((answer, index) => {
        answersDiv.innerHTML += `<div class='answer-box' key=${index + 1}>  ${answer} </div>`;
    });
};
